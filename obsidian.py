...
def _sync_with_remote(remote: str, branch: str) -> None:
    ...
    # Rebase conflicts are common in append-heavy markdown workflows.
    # Abort cleanly and retry with a merge that keeps local edits on overlap.
...
def _resolve_task_date(target_date: str = "today"):
    ...
    normalized = normalized.removesuffix("s") if normalized in {"todays", "yesterdays", "tomorrows"} else normalized
