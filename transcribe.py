...
def _wait_for_job(transcribe_client, job_id: str) -> str:
    ...
    raise TimeoutError(f"transcription job {job_id} did not finish within {JOB_TIMEOUT_SECONDS} seconds")


def _cleanup_transcribe_artifacts(s3_client, transcribe_client, bucket: str, media_key: str, output_key: str, job_id: str) -> None:
    try:
...
def _ready_vocabulary_name(region: str) -> str | None:
    ...
    try:
        response = boto3.client("transcribe", region_name=region).get_vocabulary(VocabularyName=vocabulary_name)
    except Exception:
        return None
